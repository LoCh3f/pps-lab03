...
object Sequences: // Essentially, generic linkedlists
  ...
  object Sequence:
    ...
    def skip[A](s: Sequence[A])(n: Int): Sequence[A] = (s, n) match
      case (Nil(),_) => Nil()
      case (s,0) => s
      case (Cons(head,tail),_) => skip(tail)(n-1)
    ...
    def zip[A, B](first: Sequence[A], second: Sequence[B]): Sequence[(A, B)] = (first, second) match
      case (Cons(head,tail),Cons(head2,tail2)) => Cons((head,head2),zip(tail,tail2))
      case (_,_) => Nil()
    ...
    def flatMap[A, B](s: Sequence[A])(mapper: A => Sequence[B]): Sequence[B] = s match
      case Nil() => Nil()
      case Cons(head,tail) => concat(mapper.apply(head),flatMap(tail)(mapper))
